...
pub fn execute(archive_dir: &Path) -> Result<(), String> {
    let archive = Archive::open(archive_dir)?;
    execute_internal(&archive, &mut std::io::stdout())
}
...
fn execute_internal<W: Write>(archive: &Archive, writer: &mut W) -> Result<(), String> {
